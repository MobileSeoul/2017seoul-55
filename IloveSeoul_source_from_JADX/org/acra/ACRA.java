...
public class ACRA {
    ...
    static class C01651 implements OnSharedPreferenceChangeListener {
        C01651() {
        }
    ...
    public static void init(Application app) {
        ...
        if (mReportsCrashes == null) {
            log.mo294e(LOG_TAG, "ACRA#init called but no ReportsCrashes annotation on Application " + mApplication.getPackageName());
            return;
        }
        SharedPreferences prefs = getACRASharedPreferences();
        try {
            checkCrashResources();
            log.mo292d(LOG_TAG, "ACRA is enabled for " + mApplication.getPackageName() + ", intializing...");
            ...
            log.mo302w(LOG_TAG, "Error : ", e);
        }
        mPrefListener = new C01651();
    ...
    public static ACRAConfiguration getConfig() {
        if (configProxy == null) {
            if (mApplication == null) {
                log.mo301w(LOG_TAG, "Calling ACRA.getConfig() before ACRA.init() gives you an empty configuration instance. You might prefer calling ACRA.getNewDefaultConfig(Application) to get an instance with default values taken from a @ReportsCrashes annotation.");
            }
