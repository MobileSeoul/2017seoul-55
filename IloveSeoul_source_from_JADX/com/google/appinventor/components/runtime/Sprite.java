...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
    ...
    public void PointTowards(Sprite target) {
        Heading(-Math.toDegrees(Math.atan2((target.m24Y() - m24Y()) + ((double) ((target.Height() - Height()) / 2)), (target.m22X() - m22X()) + ((double) ((target.Width() - Width()) / 2)))));
    }
    ...
    public void PointInDirection(double x, double y) {
        Heading(-Math.toDegrees(Math.atan2((y - m24Y()) - ((double) (Height() / 2)), (x - m22X()) - ((double) (Width() / 2)))));
    }
    ...
    public BoundingBox getBoundingBox(int border) {
        return new BoundingBox(m22X() - ((double) border), m24Y() - ((double) border), ((m22X() + ((double) Width())) - DEFAULT_Z) + ((double) border), ((m24Y() + ((double) Height())) - DEFAULT_Z) + ((double) border));
    }
